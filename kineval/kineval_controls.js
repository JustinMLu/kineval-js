...
kineval.applyControls = function robot_apply_controls(curRobot) {
    ...
    for (x in curRobot.joints) {
        ...
        if (curRobot.joints[x].limit !== undefined) {
            // max(a+ϵ,min(i,b−ϵ))
            curRobot.joints[x].angle = Math.max(curRobot.joints[x].limit.lower, Math.min(curRobot.joints[x].angle, curRobot.joints[x].limit.upper));
        }
